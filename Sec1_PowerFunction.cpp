...
/*
    Function descreption ligne 172
*/
...
int main(){
    ...
    */
    ...
    The matrix 𝐴 is small in size, so it is quick to compute its powers. However, to compute the n-th power of a matrix of size N*N,
        ...
        We can improve the algorithm by using the fast_power function, 
            which takes the same input and output types as slow_power but operates on a different recurrence principle. 
